...
- Sidebar account group: the `Account` nav in `shell_sidebar.html` above the account footer, and the account menu inside that footer, both range over `accountPages` (`uiAccountPages`, a list of `uiAccountPage` with `View`, `Label`, `Path`, and Lucide `Icon`). Each account panel sets `data-sidebar-view` to its page's `View`, and its handler renders through `renderUIAccountPage` so the matching link is marked current.
